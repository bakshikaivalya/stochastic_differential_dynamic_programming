%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%  Discrete Dynamic Programming for Inverted Pendulum Rigid Body Dynamics  %%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     X(:,:,j) = zeros(2,2) * dt;
   ...
   L_k(:,:,j)= - inv_H * G;
   l_k (:,j) = - inv_H *g;
   % Box constrained controls by clamping controls (Tassa, Todorov, Mansard (ICRA 2014))
   l_k (:,j) = min(max(l_k (:,j) + u_k(:,j),lb),ub) - u_k(:,j);
   if min(max(l_k (:,j) + u_k(:,j),lb),ub) == lb
       L_k (:,1,j) = 0;
       L_k (:,2,j) = 0;
   end
...
subplot(2,1,1)
hold on
plot(1:1:Horizon-1,u_new,'linewidth',4);  
title('u','fontsize',20); 
xlabel('Time in sec','fontsize',20)
hold off;
grid;
...
subplot(2,1,2)
hold on
u_stored = matfile('u_stored.mat');
u_stored = u_stored.u_new;
plot(1:1:Horizon-1,u_stored,'linewidth',4);  
title('u','fontsize',20); 
xlabel('Time in sec','fontsize',20)
hold off;
grid;