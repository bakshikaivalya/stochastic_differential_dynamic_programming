%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%  Discrete Dynamic Programming for Stochastic Inverted Pendulum Rigid Body Dynamics  %%%%%%%%%%%%%%%
...
%--------------------------------------------------> Samples disturbed by stochastic forces controlled
%using the optimal gains aroud the deterministic optimal control
...
for i=1:(Horizon-1)
    ...
    dx = x_sample(:,i,e) - x_traj(:,i); % Perturbation in trajectory due to noise
    du = l_k(:,i) + L_k(:,:,i) * dx; % Control Correction corresponding to Perturbation assuming full observability of pertbation
    u_new(:,i,e) = u_new(:,i) + du; % Corrected Control
    
    % computation of one step dynamics
    Fx(1,1) = x_sample(2,i,e);
    Fx(2,1) = -(b/I)*x_sample(2,i,e) + (g/l)*sin(x_sample(1,i,e));    
    G_x = [0; 1];
    f_stochastic = randn*sqrt(dt); % white noise stochastic force
    x_sample(:,i+1,e) = x_sample(:,i,e) + Fx * dt + G_x * u_new(:,i,e) * dt + [0; f_stochastic];
