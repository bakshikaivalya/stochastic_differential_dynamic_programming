%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%  Differential Dynamic Programming for Cart Pole Rigid Body Dynamics %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     w
    [F3_x2x2, F3_x4x4, F3_uu, F3_x2x4, F3_x4u, F3_ux2] = funcSecond_Partial_Derivatives_3varEst(@funcF3_EstDyn,w,x_traj(2,j),x_traj(4,j),u_k(:,j));
    F3_xx = zeros(4,4);
    F3_xx(2,2) = F3_x2x2;
    F3_xx(2,4) = F3_x2x4;
    F3_xx(4,2) = F3_x2x4;
...
%     [F4_x2x2, F4_x4x4, F4_uu, F4_x2x4, F4_x4u, F4_ux2] = funcSecond_Partial_Derivatives_3var(@funcF4_Dynamics,x_traj(2,j),x_traj(4,j),u_k(:,j));
    [F4_x2x2, F4_x4x4, F4_uu, F4_x2x4, F4_x4u, F4_ux2] = funcSecond_Partial_Derivatives_3varEst(@funcF4_EstDyn,w,x_traj(2,j),x_traj(4,j),u_k(:,j));
    F4_xx = zeros(4,4);
    F4_xx(2,2) = F4_x2x2;
    F4_xx(2,4) = F4_x2x4;
    F4_xx(4,2) = F4_x2x4;
...
% Regression for system parameters assuming knowledge of the equation
% structures
% if(k < num_iter)
for i=1:(Horizon-1)
Data(:,(k-1)+i) = [x_traj(2,i) x_traj(4,i) u_new(i)];
end
% end
% size(Data)
[w, par] = funcParRegr(1, Data);
w_reg(:,k) = w;
...
%---------------------------------------------> Plot Section
   ...
   plot(1:1:num_iter,w_reg(1,:), 1:1:num_iter,w_reg(2,:)*10, 1:1:num_iter, ...
       w_reg(3,:),'linewidth',2);
   plot(1:1:num_iter,1*ones(1,num_iter),'red',1:1:num_iter,0.1*ones(1,num_iter),'red',1:1:num_iter,0.01*l*ones(1,num_iter),'red','linewidth',2)
   xlabel('Iterations','fontsize',20)
   legend('m_c', 'm_p', 'm_p l')
   title('Cost','fontsize',20);